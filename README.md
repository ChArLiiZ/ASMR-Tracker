...
- **Custom Playlists** — YouTube-style playlists with custom names, colors, and emoji icons
- **One-Click Tracking** — add items to playlists directly from list pages and detail pages
- **Floating Panel** — search, filter by playlist, sort by date/title/manual order, batch operations
- **Drag & Drop** — reorder playlists in the manager, reorder items in manual sort mode
- **Emoji Icon Picker** — categorized emoji picker for playlist icons
- **Keyboard Shortcuts** — press `1`-`9` on detail pages to toggle playlists, `0` to remove all
- **Cloud Sync** — sync data across devices via private GitHub Gist
- **Smart Scanner** — auto-detects list pages, detail pages, and related sections; skips comment areas
- **Similar Title Warning** — warns when a similar title is already tracked
- **Export / Import** — JSON export & import with merge preview
- **Dark Glass Theme** — glassmorphism dark UI that blends with the site
